...
import (
	...
	"github.com/diycoder/elf/config/loader"
	"github.com/diycoder/elf/config/reader"
	"github.com/diycoder/elf/config/source"
	"github.com/diycoder/elf/config/source/file"
)
...
var (
	Loaders  []MakeLoader
	Watchers map[string]MakeWatcher
)

func init() {
	Loaders = make([]MakeLoader, 0)
	Watchers = make(map[string]MakeWatcher)
}

// Watchers a list of watcher
type MakeWatcher func() SourceWatcher
...
type MakeLoader func() Loader

type Loader interface {
	// Load makes user can load the config
	// param 0 is the config path or file
	Load() error
	Name() string
}
